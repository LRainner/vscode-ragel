...
class RagelDefinitionProvider implements vscode.DefinitionProvider {
    provideDefinition(
        ...
        const variableRegex = new RegExp(`\\b${word}\\s*=`, 'g');
...
class RagelHoverProvider implements vscode.HoverProvider {
    provideHover(
        ...
        for (const op in operatorDocs) {
            if (lineText.includes(op) && lineText.indexOf(op) <= position.character && 
                lineText.indexOf(op) + op.length >= position.character) {
